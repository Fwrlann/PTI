...
// Rota para registrar dados de saúde do usuário
...
// Rota para obter o histórico de saúde do usuário
router.get("/", authenticateToken, (req, res) => {
    const user_id = req.user.id;

    db.all("SELECT * FROM health_data WHERE user_id = ? ORDER BY date DESC", [user_id], (err, rows) => {
        if (err) {
